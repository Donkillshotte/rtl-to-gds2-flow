...
export const bilingualGlossary: BilingualGlossaryTerm[] = [
  ...
  g("CPA", "Chip–Package Analysis", "Modelli die+package (+board) per dynamic IR e |Z(f)|: SPM lumped, extracted RLC/S-param, CPS. Usati in RedHawk/Voltus.", "Die+package (+board) models for dynamic IR and |Z(f)|: lumped SPM, extracted RLC/S-param, CPS. Used in RedHawk/Voltus.", "Power", "Power"),
  g("System PDN", undefined, "Catena VRM→board→package→bump→mesh→rail→pin. Budget IR/|Z| end-to-end, non solo mesh on-die.", "Chain VRM→board→package→bump→mesh→rail→pin. End-to-end IR/|Z| budget, not on-die mesh alone.", "Power", "Power"),
  g("SPM", "Simple Package Model", "Modello package lumpato RLC per early CPA. Veloce ma debole su multi-resonance; signoff richiede extracted.", "Lumped RLC package model for early CPA. Fast but weak on multi-resonance; signoff needs extracted models.", "Power", "Power"),
  g("Z-target", "Target Impedance", "Z_target = ΔV_allowed/ΔI_step. Guida |Z(f)| di board/package/die fino alla banda on-die.", "Z_target = ΔV_allowed/ΔI_step. Guides board/package/die |Z(f)| into the on-die band.", "Power", "Power"),
