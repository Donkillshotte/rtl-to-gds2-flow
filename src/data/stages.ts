...
export const stages: Stage[] = [
  {
    ...
    deepDive: [
      ...
      "DFT readiness: ogni FF scan-able (SDFF), lock-up sui crossing di chain, OCC per at-speed, MBIST wrap su ogni SRAM (o untestable documentato), IEEE 1149.1 sui pin TCK/TMS/TDI/TDO. Occupazione tipica scan +5–12% area. Non si 'aggiunge scan in P&R'.",
    ],
    inputs: [
  ...
  },
  {
    ...
    deepDive: [
      ...
      "Lo SPEF di signoff deve includere il fill reale (grounded vs floating). Il dummy fill non è elettricamente dummy: Cground e coupling spostano il WNS di decine di ps.",
    ],
  ...
  },
  {
    ...
    deepDive: [
      ...
      "Mode DFT nello stesso MMMC: scan_shift (hold indipendente da Tshift — lock-up + delay), capture at-speed (setup a f funzionale), mbist. DRV=0 prima di credere al WNS. Temp inversion: a 7 nm setup può essere SS-cold. IR-aware: voltage map per istanza.",
    ],
  ...
  },
  {
    ...
    deepDive: [
      ...
      "DFM signoff sul deck foundry, non sul counter P&R: coloring LELE (odd cycle = fail), via doubling su clock/PG, density window, litho/PV-band, antenna cumulativa. Fill merged cambia C — SPEF di signoff dopo il fill.",
    ],
  ...
  },
  {
    ...
    deepDive: [
      ...
      "Metal-only ECO: spare già nel FEOL + metal/via (AND2 = NAND2+INV). Nuova cella in un hole slitta BTO. GKC = log tool sul database finale (STA, Calibre, IR/EM, ATPG, UPF, PKG). Un veto blocca. Dopo MTO l'unico undo è un respin.",
    ],
    inputs: [
