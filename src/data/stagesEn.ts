...
export const stagesEnMap: Record<StageId, StageText> = {
  rtl: {
    ...
    deepDive: [
      ...
      "DFT readiness: every FF scan-able (SDFF), lock-ups on chain crossings, OCC for at-speed, MBIST wrap on every SRAM (or documented untestable), IEEE 1149.1 on TCK/TMS/TDI/TDO. Typical scan overhead +5–12% area. Scan is not 'added in P&R'.",
  ...
  layout: {
    ...
    deepDive: [
      ...
      "Signoff SPEF must include real fill (grounded vs floating). Dummy fill is not electrically dummy: Cground and coupling move WNS by tens of ps.",
    ],
  ...
  sta: {
    ...
    deepDive: [
      ...
      "DFT modes in the same MMMC: scan_shift (hold independent of Tshift — lock-up + delay), at-speed capture (setup at functional f), mbist. DRV=0 before you believe WNS. Temp inversion: at 7 nm setup can be SS-cold. IR-aware: per-instance voltage map.",
    ],
  ...
  pv: {
    ...
    deepDive: [
      ...
      "DFM signoff on the foundry deck, not the P&R counter: LELE coloring (odd cycle = fail), via doubling on clock/PG, density windows, litho/PV-band, cumulative antenna. Merged fill changes C — signoff SPEF after fill.",
    ],
  ...
  tapeout: {
    ...
    deepDive: [
      ...
      "Metal-only ECO: spares already in FEOL + metal/via (AND2 = NAND2+INV). A new cell in a hole slips BTO. GKC = tool logs on the final database (STA, Calibre, IR/EM, ATPG, UPF, PKG). One veto blocks. After MTO the only undo is a respin.",
    ],
    inputs: [
